...
async def execute_crew_task(input_data: str) -> str:
    ...
    # Create a structured input with metadata
    timestamp_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC")
    enhanced_input = (
        f"{input_data}\n\n"
        f"[Query initiated at {timestamp_str}]"
    )
    
    crew = ResearchCrew(logger=logger)
    inputs = {"text": enhanced_input}
    result = crew.crew.kickoff(inputs)
    
    execution_time = time.time() - start_time
    logger.info(f"CrewAI task completed successfully in {execution_time:.2f} seconds")
    
    # Enhance result with metadata
    if hasattr(result, 'raw'):
        result_output = result.raw
    else:
        result_output = str(result)
    
    # Add execution metadata to the output
    execution_metadata = f"""

═══════════════════════════════════════════════════════════════
EXECUTION METADATA
═══════════════════════════════════════════════════════════════
Query Time: {timestamp_str}
Execution Time: {execution_time:.2f} seconds
Input Query: {input_data[:200]}{'...' if len(input_data) > 200 else ''}
═══════════════════════════════════════════════════════════════

"""
    
    # Combine the result with metadata
    enhanced_result = result_output + execution_metadata
...
async def handle_payment_status(job_id: str, payment_id: str) -> None:
    ...
    try:
        ...
        # Execute the AI task with the full input data dict
        input_text = jobs[job_id]["input_data"]["text"] if isinstance(jobs[job_id]["input_data"], dict) else str(jobs[job_id]["input_data"])
        result = await execute_crew_task(input_text)
        
        # Log the result length for debugging
        result_length = len(result)
        logger.info(f"Crew task completed for job {job_id}. Result length: {result_length} characters")
        
        # The result is already a string with metadata from execute_crew_task
...
async def get_status(job_id: str):
    ...
    # Handle both CrewOutput objects and plain strings
    if result_data:
        ...
        "input_data": [
            {
                ...
                "name": "Research Query",
                "data": {
                    "description": "Enter your research query. Can be about cars/vehicles (makes, models, trims, engines, comparisons) or general topics.",
...
def main():
    ...
    print("\nOptions:")
    print("  1. Test locally without payments:")
